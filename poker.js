...
// let keys = [ key_F, key_J, key_K, key_L, key_semicolon, ]
let keys = [ key_H, key_J, key_K, key_L, key_semicolon, ]
...
/* The letious video poker games that are supported */
...
recognize =() => {
	...
	let min = INVALID
	let tmp  = []
	let st, fl 	/* both are auto-initialized to 0 */
...
main = () => {
	...
	for(cnt = (process.argv.length-1); cnt > 1;)
	{
			...
			process.argv[2+ai].length == 3)
		{
			...
			{
				printf(stdout,"Poker: bad number given with the -is option.\n")
				process.exit(1)
			}
			...
			process.argv[2+ai].length == 2)
		{
			...
			{
				// complain and exit
				printf(stdout,"Poker: the string given with the -k option is the wrong length.\n")
				process.exit(1)
			}
			...
			for(i = 0; i < NUMKEYS; i++)
			{
				if(arg[i] == 'q' || arg[i] == 'e')
				{
					printf(stdout,"Poker: for the -k option, the string may not contain q or e.\n")
					process.exit(1)
				}
			...
			process.argv[2+ai].length > 2)
		{
			switch(process.argv[2+ai][2])
			{
				...
				default:
					printf(stdout,"Poker: digit %d given with -u option is out of range.\n",process.argv[2+ai][2]-'0')
					process.exit(1)
			}
			/* advance to next argument */
			ai += 1
		...
		/* unrecognized option */
		printf(stdout,"Poker: the %s option was not recognized\n",process.argv[2+ai])
		process.exit(1)
	}
